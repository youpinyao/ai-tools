...
- 产生：内嵌识别标记 `AI_TOOLS_PROPOSE_WORKTREE_ASK_ALWAYS_V1`、`AI_TOOLS_PROPOSE_WORKTREE_REUSE_V1`、`AI_TOOLS_PROPOSE_WORKTREE_NO_DOWNGRADE_V1`
...
def check(path: Path) -> str:
    ...
    for marker in [
        ...
        'AI_TOOLS_PROPOSE_WORKTREE_REUSE_V1',
...
预期：D 节注入同时包含 submodule 守卫、linked worktree 复用、忽略检查、禁止 hard reset、禁止静默降级、未提交改动不会自动进入新 worktree。
...
5. 已处于 linked worktree 则复用、不嵌套
6. submodule 不误判
7. 未忽略 / 创建失败 / 基线失败时，写入 change 前停止
8. 重复注入不产生重复块；缺内嵌标记为 `STALE` 并整体替换
...
3. **名称一致：** 主标记固定为 `AI_TOOLS_PROPOSE_WORKTREE_V1`；内嵌标记三枚名称在 Task 1–4 中保持不变；目标文件始终是 `opsx-propose.md` 与 `openspec-propose/SKILL.md`。
