...
在 `evidence-driven` 的 verification 阶段，把代码审查定为必做检查：规划写入 `verification.md`，apply 时实际阅读本次变更的实现 diff 并记录结论。不恢复官方 archive 裁决，不新增 Cursor rule 或 wrapper skill。

## 行为

- 模板新增一级章节「代码审查」：审查范围、审查标准（正确性、可维护性、安全、边界与测试）、发现项、结论。
- 该检查同时出现在「需求与验证对应关系」规划与「实际执行结果」。
- 规划阶段状态为「待执行」，不得预填通过。
- apply 必须实际阅读 diff 后再改状态。
- 存在未处理的 Critical 或 Important → 结论为失败，并记入剩余风险。
- Minor 可记后续项，不单独导致失败。
- 「不适用」仅当工作区相对本变更无实现 diff（纯规划、尚未编码），并写明原因。文档、schema 与配置变更仍须审查。
