# OpenSpec 最新稳定版升级实施计划

> **For agentic workers:** REQUIRED SUB-SKILL: 使用 `superpowers:subagent-driven-development`（推荐）或 `superpowers:executing-plans` 逐项执行本计划。所有步骤使用 `- [ ]` 跟踪；OpenSpec 相关对话、制品与总结必须使用简体中文。

**目标：** 在执行时将 OpenSpec CLI 升级到 npm 发布的最新稳定版，使 `evidence-driven` 与新版官方 `spec-driven` 重新对齐，并验证本仓库扩展和目标项目仍可正常工作。
...
- 不得永久假定当前基线 `1.9.0`；执行时以 `npm view @fission-ai/openspec version` 返回的最新稳定版为目标版本。
...
```bash
UPGRADE_TMP="$(mktemp -d "${TMPDIR:-/tmp}/ai-tools-openspec-upgrade.XXXXXX")"
printf '%s\n' "$UPGRADE_TMP"
```
...
```bash
OLD_SCHEMA_DIR="$(openspec schema which spec-driven)"
printf '%s\n' "$OLD_SCHEMA_DIR"
cp -R "$OLD_SCHEMA_DIR" "$UPGRADE_TMP/spec-driven-$SOURCE_VERSION"
```
...
**文件：**
- 临时读取：`$UPGRADE_TMP/spec-driven-$SOURCE_VERSION/`
...
```bash
NEW_SCHEMA_DIR="$(openspec schema which spec-driven)"
printf '%s\n' "$NEW_SCHEMA_DIR"
cp -R "$NEW_SCHEMA_DIR" "$UPGRADE_TMP/spec-driven-$TARGET_VERSION"
```
...
```bash
diff -ru \
  "$UPGRADE_TMP/spec-driven-$SOURCE_VERSION" \
  "$UPGRADE_TMP/spec-driven-$TARGET_VERSION" \
...
rg -n 'OpenSpec 1\.9\.0' \
  ...
  spec/spec-architecture-openspec-workflow-refactor.md
```
