...
**Independence gate:** This command must run in a fresh Agent conversation that did not implement the change. If the current conversation participated in implementation, stop and ask the user to open a fresh Agent conversation. The implementer cannot approve its own work.
