...
在开始改代码与派发实现子 Agent 之前，以及实现失败、证据门禁声称完成时，MUST 遵循下方写死的门禁。本节与 Spec / schema `apply.instruction` 为权威；skill/command 可更严，不得更松；冲突时以更严门禁为准。MAY Read 上游 `using-git-worktrees` / `test-driven-development` / `systematic-debugging` / `verification-before-completion` 作话术补充；**上游全文 checklist / 终态仅参考，本节下列步骤为权威**（含已写死的 worktree 检测顺序），避免与本节冲突。
...
3. **Get apply instructions**
   ...
   **Handle states:**
   ...
   CLI `apply.instruction` 与本 command 双写关键约束；冲突时以**更严门禁**为准（决策 8）。
