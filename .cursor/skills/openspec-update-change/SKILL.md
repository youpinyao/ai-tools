...
2. **Get the change's artifacts**
   ...
   **`skip_specs`:** 若 `status` 将 `specs` 标为 `skipped`，不要创建或补写 specs。纯重构类变更应在 `.openspec.yaml` 保留 `skip_specs: true`；若修订后出现规范层行为变化，先去掉该标记再引导用户用 `/opsx:propose` 续建或 `/opsx:continue` 创建 specs。
