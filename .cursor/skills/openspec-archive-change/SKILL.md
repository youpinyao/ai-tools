...
- finishing **不在** apply 中执行；verify 负责通过后的首次提示，本 skill 负责归档成功后的再次提示（若尚未集成）
...
- If sync is requested, run openspec-sync-specs inline and verify main specs before moving `changeRoot`
