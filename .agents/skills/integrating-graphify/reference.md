...
ai-tools 的对话规则、制品规则与 from-code Skill 分别只保留在 `AGENTS.md` 的
`AI_TOOLS_OPENSPEC_CONVERSATION_CHINESE_V1` 标记片段、`openspec/config.yaml`
的 `context` 标记片段和 `.agents/skills/openspec-update-change-from-code/`；
Graphify 安装或卸载时不得覆盖或删除这些内容。

## 2. 前置条件

- Python 3.10+
- `uv`（推荐）或 `pipx`
- Cursor 或 Codex（OpenSpec 与 Graphify 均提供对应集成）
...
建议提交 Cursor 规则、Codex Skill/`AGENTS.md` 片段、`.gitattributes`、`.graphifyignore`，以及 `graphify-out/` 中的三个核心图谱产物和 `manifest.json`。`cost.json` 属于本地成本记录，`cache/` 是否提交取决于仓库体积和构建速度；包含本机路径的元数据不应提交。只使用其中一种助手时，可以只提交对应平台的官方集成文件。
...
只使用其中一种助手时，只跑对应平台的安装命令即可。

各平台能力不同：Cursor 官方写入 alwaysApply 规则；Codex 官方写入 Skill 与 `AGENTS.md`（hook 为 no-op，always-on 靠 `AGENTS.md`）。个人单机使用可跳过 Git hook；需要提交共享图谱或处理并行修改的团队项目应安装 hook。随后构建图谱：
...
上述命令会分别移除 Cursor 官方规则、Codex 官方 Skill/`AGENTS.md` 片段、Git hook、merge driver 配置及其 `.gitattributes` 条目。随后删除 `.graphifyignore` 和 `graphify-out/`；若安装过程曾中断，再检查并清理残留的 `.cursor/rules/graphify.mdc` 与 `.codex/skills/graphify/`。只删除 Graphify 标记片段，不覆盖整个 `AGENTS.md`，也不删除其中 ai-tools 的 `AI_TOOLS_OPENSPEC_CONVERSATION_CHINESE_V1` 标记块或 `openspec/config.yaml` 中的 `AI_TOOLS_OPENSPEC_CHINESE_V1` 标记块。移除 Graphify 不影响 OpenSpec change、主规格或历史验证记录。
